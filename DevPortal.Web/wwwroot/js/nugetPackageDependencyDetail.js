$("#deleteNugetPackageDependency").on("click", () => {
    deleteNugetPackageDependency();
});

function deleteNugetPackageDependency() {
    const id = $("#nugetPackageDependencyId").val();
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                dataType: 'json',
                type: 'POST',
                url: Urls.DeleteNugetPackageDependency,
